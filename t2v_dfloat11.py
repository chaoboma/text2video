...
# Set up argument parser CUDA_VISIBLE_DEVICES=1 PYTORCH_ALLOC_CONF=expandable_segments:True python t2v_dfloat11.py --cpu_offload --prompt "a small boy running in a park,smiling" -output "sea.mp4"
...
# Initialize models
vae = AutoencoderKLWan.from_pretrained("Wan-AI/Wan2.2-T2V-A14B-Diffusers", subfolder="vae", torch_dtype=torch.float32, local_files_only=True)
pipe = WanPipeline.from_pretrained("Wan-AI/Wan2.2-T2V-A14B-Diffusers", vae=vae, torch_dtype=torch.bfloat16, local_files_only=True)
